...
fn bench_this(compact: &StandardCellBoard4Snakes11x11) {
    compact
        .simulate_with_moves(&[
...
fn bench_compact_full(compact: &StandardCellBoard4Snakes11x11) {
    compact.simulate(&compact.get_snake_ids()).for_each(|_| {});
}
...
fn bench_compact_repr_start_of_game(c: &mut Criterion) {
    ...
    c.bench_function("compact start of game", |b| {
        b.iter(|| bench_this(black_box(&compact)))
    });
}
...
fn bench_compact_repr_start_of_game_full(c: &mut Criterion) {
    ...
    c.bench_function("compact start of game - all moves", |b| {
        b.iter(|| bench_compact_full(black_box(&compact)))
    });
}
